...
class Network:
    ...
    def train (self, inputs, targets, epochs=30, mini_batch_size=64, learning_rate=0.01, patience=5, test_data=None):
        ...
        for full_pass in range(epochs):
            ...
            if test_data:
                ...
                for input, target in test_data:
                    result, _ = neural_network.feed_forward(input)
                    result = result[-1]
        ...
        length = len(y)
        y.insert(0, 0)
        plt.plot(list(range(0, length+1)), y)
        plt.xlabel("Epochs")
        plt.ylabel("Accuracy (%)")
...
if __name__ == "__main__":
    ...
    # neural_network.train(input_values, target_values, epochs=300,  mini_batch_size=16, learning_rate=0.1, patience=np.inf, test_data=test_data)
